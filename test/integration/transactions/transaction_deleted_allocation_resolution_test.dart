...
void main() {
  group('Deleted transaction allocation resolution', () {
    test(
      'preserves the category reference through deletion and restoration',
      () async {
        // Given
        ...
        final persistedWhileDeleted = (await transactionRepository.getById(
          transaction.id,
        )).valueOrNull!;
        expect(persistedWhileDeleted.id, transaction.id);
        expect(persistedWhileDeleted.kind, transaction.kind);
        ...
        expect(
          (await getCategoryById(category.id)).valueOrNull?.id,
          category.id,
        );
        final persistedAfterRestore = (await transactionRepository.getById(
          transaction.id,
        )).valueOrNull!;
        expect(persistedAfterRestore.id, transaction.id);
        expect(persistedAfterRestore.kind, transaction.kind);
