...
void main() {
  group('DatabaseLifecycleService', () {
    group('opening and state behavior', () {
      ...
      test('starts with isOpen false', () async {
        // Given
        final harness = await _createHarness();

        // When
        final isOpen = harness.service.isOpen;
      ...
      test(
        'isOpen becomes true only after integrity validation succeeds',
        () async {
          // Given
          final harness = await _createHarness();
          final validation = Completer<int>();
      ...
      test(
        'programmer exceptions are never wrapped in DatabaseOpenFailure',
        () async {
          // Given
          final harness = await _createHarness();
          harness.stubOpenError(StateError('programmer error'));
          ...
          expect(thrownError, isNot(isA<DatabaseOpenFailure>()));
        },
      );
    ...
    group('recovery behavior', () {
      ...
      test('successful recovery leaves isOpen true', () async {
        // Given
        final harness = await _createHarness(rawDatabases: 2);
        await harness.service.open();
    ...
    group('close behavior', () {
      ...
      test(
        'successful close changes state to closed and isOpen to false',
        () async {
          // Given
          final harness = await _createHarness();
          await harness.service.open();
