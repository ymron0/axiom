...
final class SembastMerchantRepositoryImpl implements MerchantRepository {
  ...
  Future<Result<Merchant, MerchantFailure>> archive(
    ...
    return guardPersistenceOperation<Merchant, MerchantFailure>(
      operation: () => _database.transaction<Result<Merchant, MerchantFailure>>(
        (transaction) async {
  ...
  Future<Result<void, MerchantFailure>> create(Merchant merchant) async {
    ...
    return guardPersistenceOperation<void, MerchantFailure>(
      operation: () => _database.transaction<Result<void, MerchantFailure>>((
        transaction,
      ) async {
        ...
        final persistenceModel = MerchantPersistenceModel.fromEntity(merchant);
  ...
  @override
  Future<Result<List<Merchant>, MerchantFailure>> getActive() => _readMatching(
    matches: (merchant) => !merchant.isArchived,
    failureMessage: 'Failed to read active merchants.',
  );

  @override
  Future<Result<List<Merchant>, MerchantFailure>> getAll() => _readMatching(
    matches: (_) => true,
  ...
  Future<Result<Merchant?, MerchantFailure>> getById(MerchantId id) =>
      guardPersistenceOperation<Merchant?, MerchantFailure>(
        operation: () async {
  ...
  }) => guardPersistenceOperation<List<Merchant>, MerchantFailure>(
    operation: () async {
  ...
  static MerchantPersistenceFailure _persistenceFailure(String message) =>
      MerchantPersistenceFailure(message: message);
