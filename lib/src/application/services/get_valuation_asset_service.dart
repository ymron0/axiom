...
///
/// A [SettingsNotInitializedFailure] is returned when settings have not yet
/// been initialized. An [AssetNotFoundFailure] is returned when the configured
/// valuation asset no longer exists.
  ...
  Future<Result<Asset, BaseFailure>> call() async {
    ...
    return settingsResult.when<Future<Result<Asset, BaseFailure>>>(
      success: (settings) async {
        if (settings == null) {
          return SettingsNotInitializedFailure(
